...
export const BookieAvatar = ({ level, className, size = "md", showGreeting = true, isSleeping = false }: BookieAvatarProps) => {
    ...
    const getBeeStage = () => {
        ...
        if (level === 2) return { icon: "🐝", title: "Ong Thợ", color: "text-orange-400" }; // Peach/Orange
        if (level === 3) return { icon: "🐝🍯", title: "Ong Chăm", color: "text-amber-600" }; // Amber/Diligent
        return { icon: "👑🐝", title: "Ong Chúa", color: "text-red-500" }; // Royal
    };
